...
test("createMockSession replays events on send()", async () => {
  ...
  session.on("response.output_item.added", (e) =>
    received.push(`item_added:${e.item.type}`),
  );
  session.on("response.output_text.delta", (e) =>
    received.push(`delta:${e.delta}`),
  );
  session.on("response.output_text.done", (e) =>
    received.push(`text_done:${e.text}`),
  );
  session.on("response.output_item.done", (e) =>
    received.push(`item_done:${e.item.type}`),
  );
